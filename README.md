...
Prevents the Applicant from just opening a random chat and saying "Hi".

Forces the Applicant to submit a formal Application with an introductory message.

Leaves the application in a PENDING state, invisible in the messaging tab.

If the Owner reviews the application and clicks "Accept", the system magically generates a Conversation room on the fly and drops the initial message as the first chat bubble.
...
You can use these credentials in the POST /api/v1/auth/login endpoint to retrieve a JWT Token for testing.
